...
USES_GAUSSIAN_NOISE = True
USES_ADAPTIVE_NOISE = True
CLIP_REWARD = True
...
''' QModel '''
Qw_h = init_weights([num_env_variables+num_env_actions, NETWORK_WIDTH]) # create symbolic variables
Qw_h2 = init_weights([NETWORK_WIDTH, NETWORK_WIDTH]) # create symbolic variables
Qw_h3 = init_weights([NETWORK_WIDTH, NETWORK_WIDTH]) # create symbolic variables
Qw_o = init_weights([NETWORK_WIDTH, 1])
...
Qoptimizer = tf.train.RMSPropOptimizer(learning_rate=0.002, decay=0.9)
Qtrain_op = Qoptimizer.minimize(Qcost)

''' apModel '''
apw_h = init_weights([num_env_variables, NETWORK_WIDTH]) # create symbolic variables
apw_h2 = init_weights([NETWORK_WIDTH, NETWORK_WIDTH]) # create symbolic variables
apw_h3 = init_weights([NETWORK_WIDTH, NETWORK_WIDTH]) # create symbolic variable
apw_o = init_weights([NETWORK_WIDTH, num_env_actions])
...
apOptimizer = tf.train.RMSPropOptimizer(learning_rate=0.001, decay=0.9)
aptrain_op = apOptimizer.minimize(apcost)


''' naModel '''
naw_h = init_weights([num_env_variables, NETWORK_WIDTH]) # create symbolic variables
naw_h2 = init_weights([NETWORK_WIDTH, NETWORK_WIDTH]) # create symbolic variables
naw_h3 = init_weights([NETWORK_WIDTH, NETWORK_WIDTH]) # create symbolic variable
naw_o = init_weights([NETWORK_WIDTH, num_env_actions])
...
def add_noise_TF(largeNoise = False):
    ...
    for k,v in zip(variables_names, values):
        if(k==naw_h.name):
            if USES_GAUSSIAN_NOISE:
                v2 = add_gaussian_noise(v,big_sigma,True)
            else:
