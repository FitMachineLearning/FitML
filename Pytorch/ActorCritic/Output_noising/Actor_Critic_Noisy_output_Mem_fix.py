...
class CriticModel(nn.Module):
    def __init__(self, obs_shape, action_shape,lr):
        ...
        self.net = torch.nn.Sequential(
            torch.nn.Linear(obs_shape[0]+action_shape[0],256),
            torch.nn.ReLU(),
            torch.nn.Linear(256,1) # one out put because we are predicting Q values
        )
        self.opt = optim.Adam(self.net.parameters(),lr=lr)
        if torch.cuda.is_available():
            print("Using CUDA")
...
if __name__=='__main__':
    ...
    TRAIN_CRITIC_EVERY_N_STEP = 30
    CRITIC_TRAINING_SAMPLE_SIZE = 256
    TRAIN_ACTOR_EVERY_N_STEPS = 25
