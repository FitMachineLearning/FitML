...
Update
64+64 networks
Qmodel Learning rate 0.1
AP Learning rate 0.001
150K replay memory
...
uses_project_reward_for_SM_discrimination = True #Determines if SM uses projected reward for sample discrimination
...
version_name = ENVIRONMENT_NAME+"DDPG_v0.2"
...
noise_sigma = 0.0001 #How much noise should be added to each weights
...
def add_noise(mu, largeNoise=False):
    ...
    #print ("largeNoise = ",largeNoise,"sigma =",sig)
    if x >0.5:
...
def actor_experience_replay():
    if uses_project_reward_for_SM_discrimination:
        actor_experience_replay_with_PR()
    else:
        actor_experience_replay_without_PR

def actor_experience_replay_without_PR():
    tSA = (memorySA)
    tR = (memoryR)
    tX = (memoryS)
    tY = (memoryA)
...
def actor_experience_replay_with_PR():
    tSA = (memorySA)
    tR = (memoryR)
    tX = (memoryS)
    tY = (memoryA)
    ...
    for i in range (np.alen(tR)):
        predictedReward = predictTotalRewards(tX[i], GetRememberedOptimalPolicy(tX[i]) )
        actualReward = tR[i]
    ...
    for step in range (5000):
        ...
        if done:
            ...
            if game%5 ==0 and np.alen(memoryR)>1000:
                ...
                mAverageScores.append(max(memoryR.mean(), -30)/147*100)
