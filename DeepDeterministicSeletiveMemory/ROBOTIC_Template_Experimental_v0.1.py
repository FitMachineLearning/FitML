...
num_env_variables = 10 +6
num_env_actions = 4
num_initial_observation = 0
learning_rate =  0.003
apLearning_rate = 0.001
ENVIRONMENT_NAME = "FetchReach-v0"
...
def GetRememberedOptimalPolicy(qstate):
    #print("Observation qState", qstate)
...
for game in range(num_games_to_play):
    ...
    #Get the Q state
    qsi = env.reset()
    qs = np.concatenate((qsi['achieved_goal'],qsi['desired_goal']), axis=0)
    qs = np.concatenate((qs,qsi['observation']), axis=0)
    ...
    for step in range (5000):
        ...
        si,r,done,info = env.step(a)
        s = np.concatenate((si['achieved_goal'],si['desired_goal']), axis=0)
        s = np.concatenate((s,si['observation']), axis=0)
