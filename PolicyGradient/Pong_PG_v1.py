...
weigths_filename = "Pong-PG-v0-weights.h5"
apWeights_filename = "Pong-PG-v0-weights-ap.h5"
...
if observe_and_train:
    ...
    for game in range(num_games_to_play):
        ...
        for step in range (7000):
            ...
            if game < num_initial_observation or game%2==0:
            ...
            if done:
                ...
                if len(memoryR) > 0:
                    ...
                    for i in range(0,len(gameR)):
                        tX[0] = np.asarray(gameS[i])
                        tY[0] = np.asarray(gameA[i])
                        advantage = gameR[i] - meanR
                        action_predictor_model.fit(tX.reshape(1,tX.shape[1]),tY.reshape(1,tY.shape[1]),sample_weight=advantage,epochs=1,verbose=0)
