...
version_name = "Humanoid-DDPG-v916"
...
Qmodel.add(Dense(1024*3, activation='relu', input_dim=dataX.shape[1]))
...
#model.add(Dense(num_env_variables+num_env_actions, activation='tanh', input_dim=dataX.shape[1]))
action_predictor_model.add(Dense(1024*3, activation='relu', input_dim=apdataX.shape[1]))
action_predictor_model.add(Dropout(0.2))
action_predictor_model.add(Dense(512*3, activation='relu'))
action_predictor_model.add(Dropout(0.1))
action_predictor_model.add(Dense(256*3, activation='relu'))
action_predictor_model.add(Dropout(0.1))
...
def addToMemory(reward,mem_mean,memMax,averegeReward,gameAverage,mstd):
    ...
    d_std_max = math.fabs(memMax-target)
    d_std_reward = math.fabs(reward-target)
    ...
    for game in range(num_games_to_play):
        ...
        for step in range (5000):
                ...
                for i in range(0,gameR.shape[0]):
                    ...
                    if game >3:
                        if np.alen(memoryR) > 2000:
                            atm,add_prob = addToMemory(gameR[i][0], memoryR[:2000].mean(axis=0)[0],memoryRR.max(),memoryR.mean(axis=0)[0],gameR.mean(axis=0)[0],np.std(memoryR))
                        else:
            ...
            if done:
                ...
                if game%2 ==0 and np.alen(memoryR)>1000:
                    mGames.append(game)
                    mSteps.append(step/1000*100)
                    mAPPicks.append(mAP_Counts/step*100)
                    mAverageScores.append(max(memoryR[:-1000].mean(), -40)/200*100)
