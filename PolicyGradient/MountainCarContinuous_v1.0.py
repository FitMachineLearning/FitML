...
'''
Mountain Car Continuous with Selective Memory Algorithm
...
version_name = "MountainCarC_v1.0"
...
def addToMemory(reward,mem_mean,memMax,averegeReward,gameAverage,mstd):
    ...
    prob = 0.00000005
    if gameAdvantage < 0.01 or gameAverage < averegeReward:
        ...
        for step in range (5000):
            ...
            if done :
                ...
                for i in range(0,gameR.shape[0]):
                    ...
                    if game >3:
                        if np.alen(memoryR) > 10:
                            atm,add_prob = addToMemory(gameR[i][0], pr, memoryRR.max(),memoryR[:10].mean(axis=0)[0],gameR.mean(axis=0)[0],np.std(memoryR))
                        else:
                            atm,add_prob = addToMemory(gameR[i][0], pr, memoryRR.max(),memoryR[:10].mean(axis=0)[0],gameR.mean(axis=0)[0],np.std(memoryR))
                        if add_prob < 0:
