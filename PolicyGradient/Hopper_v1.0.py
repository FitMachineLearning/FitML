...
version_name = "Hopper2D-DSMQ-v915_3xRetries_RewardHack_3of4Target_w_PR_apLR_W_gameAdv"
...
def addToMemory(reward,mem_mean,memMax,averegeReward,gameAverage,mstd):
    ...
    gameAdvantage = math.fabs((mem_mean-gameAverage)/(mem_mean-memMax))
    prob = 0.00000005
    if reward > target:
                ...
                for i in range(0,gameR.shape[0]):
                    ...
                    if game >3:
                        if np.alen(memoryR) > 2000:
                            atm,add_prob = addToMemory(gameR[i][0], pr, memoryRR.max(),memoryR.mean(axis=0)[0],gameR.mean(axis=0)[0],np.std(memoryR))
                        else:
                            atm,add_prob = addToMemory(gameR[i][0], pr, memoryRR.max(),memoryR.mean(axis=0)[0],gameR.mean(axis=0)[0],np.std(memoryR))
                        if add_prob < 0:
