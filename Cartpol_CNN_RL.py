...
This solution oberserves the first 30 games
Then plays after being trained from first 30 games.

While it plays it will take actions/policies that are sometimes random enabling
it to explore solution it hasn't yet explored.

The proability of random action reduces over each episode as it gets more and more comfortable with the rules of the game.
...
starting_explore_prob = 0.30
training_epochs = 500
num_anticipation_steps = 8
...
if load_previous_weights:
    ...
    fn = dir_path + "/CP-weights_RL_CNN.h5"
    print("filepath ", fn)
    if  os.path.isfile(fn):
        print("loading weights")
        model.load_weights("CP-weights_RL_CNN.h5")
    else:
        print("File CP-weights_RL_CNN.h5 does not exis. Retraining... ")
...
if observe_and_train:
    ...
    print("Initial training complete. Begin tentative exploration.")

'''
dataX = np.random.random((1,5))
res = model.predict(dataX[0].reshape(1,dataX.shape[1]))
nstate = res[0][:-1]
print("predicted output ", res)
print("expected reward ", res[0][4])
...
for game in range(20):
    ...
    explore_prob = starting_explore_prob-(starting_explore_prob/20.0)*game
    ...
    for step in range (300):
        ...
        if prob < explore_prob:
            ...
            #print("taking random action ", total_steps)
        ...
        qs_a = np.concatenate((qs,np.array([a])), axis=0)
        s_r = np.concatenate((s,np.array([r])), axis=0)
        #add event to training set
        dataX = np.vstack((dataX,qs_a))
        dataY = np.vstack((dataY,s_r))

        total_steps += 1
        if done and r==-1:
            #If you fail, add new knowledge to NN
            #Retrain the NN
            #break
            ...
            #retrain after every 6 failures
...
if save_weights:
    ...
    model.save_weights("CP-weights_RL_CNN.h5")
