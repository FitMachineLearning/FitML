...
Qmodel.add(Dense(1024, activation='tanh', input_dim=dataX.shape[1]))
...
#model.add(Dense(num_env_variables+num_env_actions, activation='tanh', input_dim=dataX.shape[1]))
action_predictor_model.add(Dense(1024, activation='tanh', input_dim=apdataX.shape[1]))
            ...
            if done :
                ...
                for i in range(0,gameR.shape[0]):
                    ...
                    if i==gameR.shape[0]-1:
                        print("Training Game #",game,"memoryR",memoryR.shape[0], "memoryRR ",memoryRR.shape[0],"memoryRR mean",memoryRR.mean(axis=0)[0], " steps = ", step ,"last reward", r," finished with headscore ", gameR[(gameR.shape[0]-1)-i][0])
                ...
                for i in range(0,gameR.shape[0]):
                    if game > 15 and addToMemory(gameR[i][0],memoryRR.mean(axis=0)[0]):
