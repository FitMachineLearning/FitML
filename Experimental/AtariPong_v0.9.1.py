...
Using DeepQ N
achieves 50% score in 14 night training
'''
import numpy as np
import keras
import gym
...
memoryA = np.zeros(shape=(1,num_env_actions))
    ...
    for game in range(num_games_to_play):
        ...
        for step in range (7000):
            ...
            if done:
                ...
                for i in range(0,gameR.shape[0]):
                    ...
                    if i==gameR.shape[0]-1:
                        print("Training Game #",game,"#scores", num_points, " total # scores ", num_games_won,"avg scores per match ",num_games_won/(game+1), "memory ",memoryR.shape[0]," finished with headscore ", gameR[(gameR.shape[0]-1)-i][0])
            ...
            if done and game >= num_initial_observation:
                if game%15 == 0 and game>10:
                    print("Training  game# ", game,"momory size", memorySA.shape[0])

                    #training Reward predictor model
                    model.fit(memorySA,memoryR, batch_size=mini_batch,epochs=training_epochs,verbose=2)
