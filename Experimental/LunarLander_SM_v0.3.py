...
#Qmodel.add(Dense(64, activation='sigmoid'))
...
def predictTotalRewards(qstate, action):
    #qs_a = np.concatenate((qstate, to_one_hot(action,num_env_actions)), axis=0)
...
if observe_and_train:
    ...
    for game in range(num_games_to_play):
        ...
        for step in range (5000):
            ...
            #print("a before rounding",a)
            a = np.around(a)
            a = a.astype(int)
            #print("a after rounding",a)
            ...
            if done :
                ...
                for i in range(0,gameR.shape[0]):
                    #get expected Q
                    #print("gameS and GameA",gameS[i],gameA[i])
                    expectedQ = predictTotalRewards(gameS[i],gameA[i])
                    if game > 3 and addToMemory(gameR[i][0], expectedQ, memoryRR.max()):
            ...
            if done and game >= num_initial_observation and game >= 3:
                if game%2 == 0:
                    if game%25 == 0:
